...
public class RolController {
    ...
    @PatchMapping("/{id}")
    @Operation(summary = "actualizar parcialmente un rol", description = "permite modificar solo algunos campos sin reemplazar todo el objeto.")
    public ResponseEntity<Rol> actualizarParcial(
            @PathVariable Integer id, @RequestBody Rol cambios) {

        Rol actualizado = rolService.actualizarParcial(id, cambios);
        if (actualizado == null)
            return ResponseEntity.notFound().build();
        return ResponseEntity.ok(actualizado);
    }
